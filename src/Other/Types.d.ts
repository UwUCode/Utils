...
export type FilterReturnType<Base, Condition> = {
	[Key in keyof Base]: Base[Key] extends AnyFunction ? ReturnType<Base[Key]> extends Condition ? Key : never : never;
};
export type InvertedFilterReturnType<Base, Condition> = {
	[Key in keyof Base]: Base[Key] extends AnyFunction ? ReturnType<Base[Key]> extends Condition ? never : Key : never;
};
...
export type WithoutFunctions<T> = BetterFilter<T, AnyFunction>;
...
export type ThenReturnType<T extends AnyFunction> = ThenArg<ReturnType<T>>;
